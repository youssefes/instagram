...
class CommentCell: UICollectionViewCell {
    ...
    var comment : Comment?{
        didSet{
            ...
            let nsAttributedString = NSMutableAttributedString(string: userName, attributes: [NSAttributedString.Key.font : UIFont(name: Font.Medium.name, size: 14) ?? ""])
            nsAttributedString.append(NSAttributedString(string: "  " + commet.text, attributes: [NSAttributedString.Key.font : UIFont(name: Font.ExtraReg.name, size: 18) ?? ""]))
